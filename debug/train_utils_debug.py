...
def Train(m, train_loader: DataLoader, val_loader: DataLoader, optimizer, eval_interval, device):
    ...
    t0 = time.time()
    optimizer.zero_grad(set_to_none=False)  # clear old gradients efficiently
    lossi = []
    normi = []
    accum_steps = config.macro_batch_size // config.batch_size # gradient accumulation steps
    print(f"for this run, accum step is {accum_steps}", flush=True)
    loss_accum = 0.0
    macro_batch_count = 0
    micro_batch_count = 0
    epoch_idx = 0
    ...
    while macro_batch_count < config.max_steps:
        ...
        for (X, Y) in train_loader:
            ...
            if micro_batch_count % accum_steps == 0:
                ...
                optimizer.zero_grad(set_to_none=False)  # clear gradients for next step
                loss_value = loss_accum.item()
                lossi.append(loss_value)
                print(f"batch size is {config.batch_size}, initial loss is {loss_value:3g}",flush=True)
                loss_accum = 0.0
